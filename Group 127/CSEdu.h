...
#include <stdexcept>
#include "Tree\Tree.h"
#include "Operations\biops.h"

using namespace std;

// defines different types of nodes that can exist in the CSE machine.
...
bool isOperator(const string &label);//check whether a given string represents an operator.

class CSENode//represents a node in the CSE machine.
{
private:
    // General node properties
    ObjectType nodeType;//An enumeration representing the type of the node (e.g., lambda, identifier, integer).
    string nodeValue;//A string representing the value of the node.

    // lambda and eeta nodes
    int env{};//A string representing the value of the node.
    int csIndex{}; // An integer representing the control structure index. It's used for delta, tau, eeta, and lambda nodes.
    vector<string> boundVariables;//A vector of strings representing the bound variables for lambda nodes.
    vector<CSENode> listElements;//A vector of CSENode objects representing the elements of a list node.
    bool isSingleBoundVar = true;//A boolean flag indicating whether the lambda node has a single bound variable or multiple bound variables.
...
public:
    ...
    // getters
    ObjectType get_NodeType() const { return nodeType; }// It returns an ObjectType enum value representing the type of the node.

    string get_nodeValue() const { return nodeValue; }//returns a string representing the value associated with the node.

    int get_ENV() const { return env; }//returns an integer representing the environment of the node.

    int get_CSIndex() const { return csIndex; }//returns an integer representing the control structure index of the node.

    bool get_IsSingleBoundVar() const { return isSingleBoundVar; //returns a boolean indicating whether the node has a single bound variable or not.
    vector<string> get_varList() const { return boundVariables; }// returns a vector of strings representing the bound variables of the node, if any.

    vector<CSENode> get_ListElements() const { return listElements; }//returns a vector of CSENode objects representing the elements of a list node, if applicable.
